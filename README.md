...
A critical component of a nitrogen generator is a needle valve which is placed post nitrogen membrane. This needle valve controls the resulting nitrogen purity. Too high air flow at the membrane output will result in high oxygen levels or no oxygen filtering at all. Too low and most of the compressed air input is lost in the permeate and the efficiency of the system drops off a cliff. This adjustment is highly dependent on the type and size of membrane used and drive the required continuous compressed air requirement.
...
Typically a home brewer will not need that much nitrogen. Most commercial large scale nitrogen generation equipment is built for continuous N2 blanketing and is optimized for that use case. We need to carefully consider what our goals are:
...
- easy maintainability overall
- reasonable N2 output for the application
...
- Max system pressure of about 100psi. That way we can use a Cornelius keg as a holding tank. 
...
- Experience with PLCs or building your own PCB / Arduino to control the air system. Some programming is required.
...
\
Membranes have a 'knocking' limit, i.e. maximum pressure differential going into the system when suddenly opening a valve. This is usually 20-50psi. A too high pressure differential can shatter the membrane, completely destroying it. We will control this through a needle valve on the input side. Most commercial equipment will omit this valve since those systems are usually in a well controlled environment. I assume the user (me) will make a stupid mistake at some point and I'd rather protect the expensive membrane. \
\
...
2. We need a 3-stage air filter to treat the air. The stages are usually a 1μm particulate filter, a 0.1μm oil filter and a 0.01μm coalescing filter. There are many variations on that but the key is that we want a 0.01μm stage in there somewhere. If your air contains a lot of hydrocarbons (like in an active workshop) you'd also want an activated charcoal filter in addition to the 3-stage filter. Prices range from $200 to $2000 (if you get one on McMaster). I chose a PneumaticPlus SAFU460-N04-MEP which seemed reasonably priced at $200. Do NOT skimp on this important component! Nitrogen membranes can last for years, improperly filtered air can reduce that to mere hours! We are talking atomic level membrane filters here.

[<img src="./images/airfilter.jpg" width="100"/>](./images/airfilter.jpg)

3. Most commercially available air compressors can easily provide the air required for this membrane. I chose a Makita MAC100Q. It's small and quiet. About $250.
...
5. A holding tank for the nitrogen. A 5 gallon Cornelius will do, $130.

[<img src="./images/corneliouskeg.jpeg" width="100"/>](./images/corneliouskeg.jpeg)

6. We need a controller to sense the air pressures and drive the solenoids. My initial prototype accomplished this with a Click PLC from Automation Direct. It's rather bulky, requires ladder logic knowledge and a complete system comes to about $300. For my final product I built my own custom PCB which is way more compact. The DigiKey BOM for this is about $130, 5 prototype PCBs from PCBWay cost me $50 including shipping. I added an LCD screen for status display as a bonus. Apart from a PLC or custom PCB, a hacked together Arduino project will also work.
...
Total cost (assuming one membrane) was $2222. That's still only about half the price of a commercial solution!
...
Some timers (5-10s) are used to stagger the activation of the solenoids (hysteresis) to avoid oscillations and feedback loops.
...
- If the input pressure (AirPressure1) increases beyond 120psi at any point, both solenoids stay closed. Too high of pressure can destroy the membrane.
- If the input and output pressures are out of range (<0 or >150psi) (AirPressure1 and NitrogenPressure1), both solenoids stay closed. This could happen when one of the pressure sensors fail.

### Procedure to adjust input and output needle valves

#### Preparation

1. The input needle valve should be adjusted so that suddenly feeding 100psi through it does not cause a "loud bang". Rather it should feel like 5-10 psi coming out of a compressor. Rig something up and adjust that valve before you put the entire system together.
...
6. Stop right there. Closing it more will cause the permeate air to nitrogen air ratio to go above the ideal, i.e. much higher than 5:1 and will not improve nitrogen concentration further. 
