...
class ST7525Display {
   public:
    ...
    void draw_char(uint8_t x, uint8_t y, char c) {
        if (x > COLUMNS - 8 || y > LINES - 8 || c < 32 || c > 127)
            return;
        const uint8_t *chr = font8x8_basic[c];
        for (uint8_t col = 0; col < 8; ++col) {
            uint8_t bits = chr[col];
            for (uint8_t row = 0; row < 8; ++row) {
                set_pixel(x + row, y + col, (bits >> row) & 1);
            }
        }
    }

    void draw_text(uint8_t x, uint8_t y, const char *str) {
        while (*str && x <= COLUMNS - 8) {
            draw_char(x, y, *str++);
            x += 8;
        }
    }

    void draw_text_2x(uint8_t x, uint8_t y, const char *str) {
        while (*str && x <= COLUMNS - 16) {
            draw_char_2x(x, y, *str++);
            x += 16;
        }
    }

    void draw_char_2x(uint8_t x, uint8_t y, char c) {
        if (x > COLUMNS - 16 || y > LINES - 16 || c < 32 || c > 127)
            return;
        const uint8_t *chr = font8x8_basic[c];
        for (uint8_t col = 0; col < 8; ++col) {
            uint8_t bits = chr[col];
            for (uint8_t row = 0; row < 8; ++row) {
                uint8_t pixel = (bits >> row) & 1;
                // scale 2x2 pixels
                set_pixel(x + row * 2 + 0, y + col * 2 + 0, pixel);
                set_pixel(x + row * 2 + 1, y + col * 2 + 0, pixel);
                set_pixel(x + row * 2 + 0, y + col * 2 + 1, pixel);
                set_pixel(x + row * 2 + 1, y + col * 2 + 1, pixel);
            }
        }
    }
   ...
   private:
    ...
    static constexpr uint8_t font8x8_basic[128][8] = {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0000 (nul)
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim == &htim1) {
        ...
        ST7525Display::instance().draw_text_2x(0, 0, output);
    }
    if (htim == &htim3) {
      static volatile int32_t c = 0;
      static char output[32] = {0};
      ...
      ST7525Display::instance().draw_text_2x(0, 16, output);
  }
